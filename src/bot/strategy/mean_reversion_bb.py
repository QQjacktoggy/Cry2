...
class MeanReversionBBStrategy(BaseStrategy):
    ...
    def __init__(self, params: dict[str, Any] | None = None) -> None:
        ...
        self.risk_pct = params.get("risk_pct", 0.5)
        self.take_profit_mode = params.get("take_profit", "bb_middle")
        self.long_only = params.get("long_only", False)
        self.max_open_positions = params.get("max_open_positions", 2)

        vol_filter = params.get("volatility_filter", {})
        self.atr_percentile = vol_filter.get("atr_percentile", 50)
    ...
    def on_bar(self, event: MarketEvent) -> list[SignalEvent]:
        ...
        if pos.is_open:
            ...
            elif not self.long_only and price >= bb_upper and rsi > self.rsi_overbought:
    ...
    def _calc_size(self, price: float) -> float:
        """Position sizing based on equity and stop-loss risk.

        Risk 1% of equity per trade. If stop_loss_pct = 1.5%,
        position size = (equity * 0.01) / (price * stop_loss_pct/100) * leverage.
        """
        if price <= 0 or self._equity <= 0:
            return 0.0
        risk_amount = self._equity * (self.risk_pct / 100.0)
