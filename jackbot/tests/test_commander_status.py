...
async def test_status_prefers_exchange_pnl_over_paper_portfolio() -> None:
    ...
    assert "交易所已實現(未扣手續費): $+0.0820 USDT" in output
    ...
    assert "未實現損益" in output
    assert "• 已實現:" not in output
    assert output.count("負PnL 成交") == 1


@pytest.mark.asyncio
async def test_status_zero_fills_does_not_duplicate_negative_pnl_label() -> None:
    status = {
        "strategy_variant": "baseline_grid",
        "mode": "aggressive",
        "safe_mode": False,
        ...
        "exchange_orphan_orders": [],
        "exchange_orphan_positions": [],
        "exchange_today_realized_pnl": 0.0,
        "exchange_today_commission": 0.0,
        "exchange_today_fills": 0,
        "session_negative_fill_count": 0,
        "session_negative_fills": [],
    }
    commander = JackbotCommander(
