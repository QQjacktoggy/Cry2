...
| 年化報酬率 | > 25% | **45.2%** | ✅ 超標 81% |
| 最大回撤 | < 20% | **-10.6%** | ✅ 通過（本次回測各版本中最佳） |
| Calmar Ratio | > 1.0 | **4.27** | ✅ 超標 327%（本次回測各版本中最佳） |
| Walk-Forward OOS Sharpe（post-selection） | > 0.5 | **2.004** | ✅ 通過，但非嚴格 nested OOS（見 §6） |
| 策略相關性（daily-resampled） | < 0.1 | **0.062** | ✅ 通過 |
